...
class DGC(nn.Module):
    def __init__(self, model, rank, size, momentum, full_update_layers, percentage):
        """Class for performing sparse distributed gradient updates before backpropagation
        :parameter
        model : torch.Sequential, main model to be trained in data-parallel distributed manner
        ...
        percentages : list of floats, percentages of sparsification
        iterations : list of ints, iterations at which percentages of sparsification will be changed
        """
    ...
    def average_gradient_dense(self):
    ...
    def accumulate_gradients(self):
        """Accumulate gradients per iterations
        :return
        ...
        amounts_per_layer : list[int], number of elements selected from each layer
        """
        gradient_tensors, gradient_indices, gradient_amounts = [], [], []
        layer_idx = 0
        ...
        for i, (name, p) in enumerate(self.main_model.named_parameters()):
            ...
            if len(p.grad.data.cpu().shape) == 4:
                top_indices, top_values = self.select_top_values_and_indices(p.grad.data.cpu(), name, self.momentum)
    ...
    def average_gradients_sparse(self, value):
        """Averaging sparse gradients obtained from all the nodes (synchronized)
        :parameter
        value : list[tensor], constructed sparse gradients of separated nodes
        :return
        ...
        for i, (name, param) in enumerate(self.main_model.named_parameters()):
            ...
            if len(param.grad.data.cpu().shape) == 4:
                ...
                if self.rank == 0:
                    ...
                    avg_grads.append(updated_grad.to('cpu'))
    ...
    def construct_grads(self, grads, indices, amounts):
        """
        Constructions from the separate indices sparse gradient tensor, with the same shape as original gradient,
        with top values at chosen indices, and zeros elsewhere
        :parameter
        ...
        indices_amounts = amounts.to('cpu')
        conv_layer_idx = 0
        for i, (name, p) in enumerate(self.main_model.named_parameters()):
            if i in self.full_update_layers:
                continue
    ...
    def gradient_update(self):
        ...
        updated_grads = self.construct_grads(gradient_vector.to('cpu'), indices=gradient_indices, amounts=gradient_amounts)
        grad_update_conv = self.average_gradients_sparse(value=updated_grads)
        updated = self.transfer_gradients(grad_update_conv=grad_update_conv)
        self.update_gradients(updated)
        self.average_gradient_dense()