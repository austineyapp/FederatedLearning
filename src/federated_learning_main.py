...
if __name__=="__main__":
    ...
    #set model to train and send to device
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        logger.add_scalar('Training Loss', np.mean(np.array(train_loss)), epoch+1)
        logger.add_scalar('Training Accuracy', 100*train_accuracy[-1], epoch+1)
        test_acc, test_loss = test_inference(args, global_model, test_dataset)
        logger.add_scalar("Test Loss", test_loss, epoch+1)
        logger.add_scalar("Test Accuracy", 100*test_acc, epoch+1)
        logger.close()
